...
class Juego {
  ...
  iluminarSecuencia() {
    for (let i = 0; i < this.nivel; i++) {
      ...
      setTimeout(() => this.iluminarColor(color), 640 * i)
      if(i === (this.nivel - 1)){
        setTimeout(()=>this.agregarEventosClick(),((640*i)+300))
      }
    }
  ...
  elegirColor(ev) {
    ...
    if (colorPickedNumber === this.secuencia[this.aciertos]) {
      ...
      if (this.aciertos === this.nivel) {
        ...
        } else if(this.nivel === (DEFAULT_LEVEL+1)){
          document.getElementById('progress-level').innerHTML = ""
         this.ganaste() 
        }else {
          if(this.nivel === 1){
  ...
  ganaste() {
    swal("Winner!", "Simon celebrates it!😊", "success").then(() => {
      
      this.inicializar();
    });
  }
  perdiste() {
    swal("You lose!", "Simon is Sad!😥", "error").then(() => {
      this.eliminarEventosClick();
